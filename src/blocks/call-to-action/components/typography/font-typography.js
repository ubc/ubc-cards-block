...
function FontFamilyControl( props ) {
			...
			if ( typeof gfontsObj === 'object' ) {
				gfontsObj.forEach( function( item ) {
					if ( fontWeight.value !== item ) {
						newValue = item;
						props.setAttributes( { [ props.fontWeight.label ]: newValue } );
					}
				} );
				...
				gfontSubsetObj.forEach( function( item ) {
					if ( fontSubset.value === item ) {
						newValue = item;
						props.setAttributes( { [ props.fontSubset.label ]: newValue } );
					}
				} );
			}
		}
	...
	return (
		<div className="uag-typography-font-family-options">
			<label htmlFor="uag-typography-font-family" className="uag-typography-font-family-label">{ __( 'Font Family' ) }</label>
			<Select
				options={ fonts }
